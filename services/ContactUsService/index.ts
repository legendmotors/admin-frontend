...
export const subscribeContact = async (payload: Record<string, any>) => {
    try {
        const response = await api.post(Apis.AddContact, payload);
        ...
        if (axios.isAxiosError(error)) {
            console.error('Error submitting enquiry:', error.response?.data?.message);
            showNotification(error.response?.data?.message || 'Error submitting enquiry.', 'error');
        } else {
...
export const getContacts = async (params: Record<string, any>) => {
    try {
        const response = await api.get(Apis.GetContactList, { params });
        if (!response || !response.data.success) {
...
export const getContactById = async (id: number) => {
    try {
        const response = await api.get(Apis.GetContactById, { params: { id } });
        if (!response || !response.data.success) {
...
export const deleteContact = async (id: number) => {
    try {
        const response = await api.delete(Apis.DeleteContact, { params: { id } });
        if (!response || !response.data.success) {
