...
const UpdateBlogCategoryComponent = ({ categoryId }: { categoryId: number }) => {
  const formikRef = useRef<any>(null);
  const router = useRouter();
  const [progress, setProgress] = useState<number>(0);
  const [statusMessage, setStatusMessage] = useState<string>('');
  const [initialValues, setInitialValues] = useState<BlogCategoryFormValues | null>(null);

  useEffect(() => {
    const fetchCategory = async () => {
      const response = await BlogCategoryService.getBlogCategoryById(categoryId);
      if (response) {
        setInitialValues({
          name: response.name,
          slug: response.slug
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const response = await BlogCategoryService.updateBlogCategory({ ...values, id: categoryId });
      setSubmitting(false);
  ...
  return (
    <div className="flex flex-col gap-6 xl:flex-row">
      <div className="panel flex-1 px-0 pb-6 ltr:xl:mr-6 rtl:xl:ml-6 pt-0">
        <SectionHeader title="Edit Blog Category" />
...
export default UpdateBlogCategoryComponent;
