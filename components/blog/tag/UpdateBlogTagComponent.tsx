...
const UpdateBlogTagComponent = ({ tagId }: { tagId: number }) => {
    ...
    useEffect(() => {
        const fetchTag = async () => {
            const response = await BlogTagService.getBlogTagById(tagId);
            if (response) {
                setInitialValues({
                    name: response.name,
                    slug: response.slug
  ...
  `;
    ...
    const handleSubmit = async (
        ...
        try {
            ...
            const response = await BlogTagService.updateBlogTag({ ...values, id: tagId });
            setSubmitting(false);
...
export default UpdateBlogTagComponent;
