    ...
    return (
      <div className="toolbar file-selected">
        <div className="file-action-container">
          <div>
            {/* <button className="btn btn-danger gap-2 mx-1" onClick={() => handleCutCopy(true)}>
              <BsScissors size={18} />
              <span>Cut</span>
            </button>
            <button className="btn btn-danger gap-2" onClick={() => handleCutCopy(false)}>
              <BsCopy strokeWidth={0.1} size={17} />
              <span>Copy</span>
            </button> */}
            {/* {clipBoard?.files?.length > 0 && (
              <button
                className="btn btn-danger gap-2 mx-1"
                onClick={handleFilePasting}
                // disabled={!clipBoard}
              >
                <FaRegPaste size={18} />
            ...
            {/* {!selectedFiles.isDirectory && (
              <button className="btn btn-danger gap-2 mx-1" onClick={handleDownloadItems}>
        ...
        <div>
          {toolbarRightItems.map((item, index) => (
            <div key={index} className="toolbar-left-items">
              <button className="btn btn-outline-primary gap-2 mx-1" title={item.title} onClick={item.onClick}>
                {item.icon}
              </button>
