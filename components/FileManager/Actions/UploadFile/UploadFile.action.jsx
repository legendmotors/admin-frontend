  ...
  const setSelectedFiles = (selectedFiles) => {
    ...
    if (selectedFiles.length > 0) {
      const newFiles = selectedFiles.map((file) => {
        const appendData = onFileUploading(file, currentFolder); // ✅ Ensure `currentFolder` (parent) is passed
        ...
        return {
          file: file,
          appendData: { ...appendData, parentId: currentFolder?.id || null }, // ✅ Explicitly add `parentId`
          ...(error && { error: error }),
        };
      });
