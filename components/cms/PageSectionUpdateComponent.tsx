...
const PageSectionUpdateComponent = ({ pageId }: { pageId: number }) => {
    ...
    const fetchPage = async () => {
      ...
      if (response) {
        setInitialValues({
          ...
          title: response?.data?.title || "", // set the title from response
  ...
  const validationSchema = Yup.object().shape({
    sectionKey: Yup.string().required("Please fill the section key"),
    title: Yup.string().required("Please fill the title"), // validation for title
  });
            ...
            {({ isSubmitting, setFieldValue, values }) => (
              <Form className="space-y-4">
                ...
                <div>
                  ...
                  <Field name="sectionKey" type="text" placeholder="Enter section key" className="form-input" />
